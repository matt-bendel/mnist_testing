...
class rcGANWRegLatent(pl.LightningModule):
    def __init__(self, args, exp_name):
        super().__init__()
        self.args = args
        self.exp_name = exp_name
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (self.global_step - 1) % self.args.pca_reg_freq == 0 and self.current_epoch >= 15:
            gens = torch.zeros(
                ...
                device=self.device)
            for z in range(self.args.num_z_pca):
                gens[:, z, :, :, :] = self.autoencoder(self.forward(y), features=True)

            gens_zm = gens - torch.mean(gens, dim=1)[:, None, :, :, :].clone().detach()
            gens_zm = gens_zm.view(gens.shape[0], self.args.num_z_pca, -1)

            x_zm = self.autoencoder(x, features=True) - torch.mean(gens, dim=1).clone().detach()
            x_zm = x_zm.view(gens.shape[0], -1)
...
class rcGANWRegJoint(pl.LightningModule):
    def __init__(self, args, exp_name):
        super().__init__()
        self.args = args
        self.exp_name = exp_name
    ...
    def l1_std_p(self, avg_recon, gens, x, betastd):
        return F.l1_loss(avg_recon, x) - betastd * np.sqrt(
            2 / (np.pi * self.args.num_z_train * (self.args.num_z_train + 1))) * torch.std(gens, dim=1).mean()
    ...
    def training_step(self, batch, batch_idx):
        ...
        g_loss = self.adversarial_loss_generator(y, gens)
        g_loss += self.l1_std_p(torch.mean(gens, dim=1), gens, x, self.betastd)
        ...
        g_loss += self.latent_weight * self.l1_std_p(avg_recon, new_gens, self.autoencoder(x, features=True), self.betastd_latent)
    ...
    def on_validation_epoch_end(self):
        ...
        mu_0_latent = 2e-4
        self.betastd += mu_0 * psnr_diff
        self.betastd_latent += mu_0_latent * psnr_diff
...
class rcGANWRegOptuna(pl.LightningModule):
    def __init__(self, args, exp_name, start_lr, beta_pca, patience, lr_step):
        super().__init__()
        self.args = args
        self.exp_name = exp_name
    ...
    def configure_optimizers(self):
        opt_g = torch.optim.Adam(self.generator.parameters(), lr=self.start_lr,
                                 betas=(self.args.beta_1, self.args.beta_2))
        opt_d = torch.optim.Adam(self.discriminator.parameters(), lr=self.args.lr,
                                 betas=(self.args.beta_1, self.args.beta_2))
