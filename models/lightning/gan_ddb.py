...
from architectures.ddbnet import Image256Net
from architectures.discriminator import DiscriminatorDDB
...
class rcGANDDB(pl.LightningModule):
    def __init__(self, args, exp_name):
        ...
        self.generator = Image256Net()

        self.discriminator = DiscriminatorDDB(
    ...
    def forward(self, x_t, t):
        z = self.get_noise(x_t.size(0))
        input = torch.cat([x_t, z], dim=1)
        samples = self.generator(input, t)
        return samples

    def compute_gradient_penalty(self, real_samples, fake_samples, x_t, t):
        """Calculates the gradient penalty loss for WGAN GP"""
        # Random weight term for interpolation between real and fake samples
        alpha = torch.rand((real_samples.size(0), 1, 1, 1)).to(self.device)
        # Get random interpolation between real and fake samples
        interpolates = (alpha * real_samples + ((1 - alpha) * fake_samples)).requires_grad_(True)
        d_interpolates = self.discriminator(interpolates, x_t, t)
    ...
    def gradient_penalty(self, x_hat, x, x_t, t):
        gradient_penalty = self.compute_gradient_penalty(x, x_hat, x_t, t)

        return self.args.gp_weight * gradient_penalty

    def adversarial_loss_generator(self, x_t, gens, t):
        fake_pred = torch.zeros(size=(x_t.shape[0], self.args.num_z_train), device=self.device)
        for k in range(x_t.shape[0]):
            ...
            temp = self.discriminator(gens[k], cond, t[k].unsqueeze(0).repeat(self.args.num_z_train, 1))
            fake_pred[k] = temp[:, 0]

        gen_pred_loss = torch.mean(fake_pred[0])
        for k in range(x_t.shape[0] - 1):
    ...
    def training_step(self, batch, batch_idx):
        ...
        x_t = (1 - self.alpha(t)[:, None, None, None]) * x + self.alpha(t)[:, None, None, None] * y

        opt_g, opt_d = self.optimizers()

        x_hat = self.forward(x_t, t)

        fake_pred = self.discriminator(x_hat, x_t, t)
        real_pred = self.discriminator(x, x_t, t)

        adv_loss_d = fake_pred.mean() - real_pred.mean()
        gp_loss_d = self.gradient_penalty(x_hat, x, x_t, t)
        ...
        for z in range(self.args.num_z_train):
            gens[:, z, :, :, :] = self.forward(x_t, t)

        avg_recon = torch.mean(gens, dim=1)

        g_loss = self.adversarial_loss_generator(x_t, gens, t)
    ...
    def on_validation_epoch_end(self):
        ...
        self.log('psnr_8', psnr_8, prog_bar=True)
    ...
    def on_train_epoch_end(self):
        ...
        sch_g.step(self.trainer.callback_metrics["psnr_8"])
